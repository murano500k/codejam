    ...
    while(getline(fin,buf) ){
        switch (next_action){
            ...
            case GENERAL:
                ...
                cout<<"Case"<<iter_t+1<<": max_cakes="<<max_cakes<<" allitems="<<items_num<<endl;
                iter_c=0;
            ...
            case ITEM:
                ...
                if(iter_c==items_num){
                    ...
                    for (int i = 0; i < items_num; ++i) {
                        ...
                        cout<<"cake"<<i<<": s="<<cakes[i].s_side<<"+"<<cakes[i].s_top<<"-"<<cakes[i+1].s_top<<endl<<endl;
                    }
                    int i=0, j=0;
                    bool skip=true;
                    while(i<items_num && j<max_cakes){
                        if(skip && (cakes[j].s>=cakes[j+1].s || items_num-i==max_cakes-j)) {
                            skip=false;
                            s_max+=cakes[j].s;
                            i++;
                        }
                    ...
                    //Get result
                    iter_t++;
                    cout<<"Case #"<<iter_t<<": "<<s_max<<endl<<endl;
                    fout<<setprecision(9)<<"Case #"<<iter_t<<": "<<s_max<<endl;
                }
                break;
        }
    }
